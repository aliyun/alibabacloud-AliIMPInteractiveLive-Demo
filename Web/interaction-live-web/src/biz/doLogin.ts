...
// 配置登录用的config
const { appKey, appId, signSecret } = roomEngineConfig;
...
export const setConfig = (
  ...
  const origin = 'https://**.xxx.com'; // 自己的server地址，配跨域，也可以代理
  ...
  const config = {
    ...
    authTokenCallback: authed
      ...
      : () => {
          ...
          // 生成签名
          const signature = SignUtils.generateSignature(
            signString,
            signSecret + '&',
          );
          return fetch(`${origin}/${path}?${queryString}`, {
            ...
            .then((res) => {
              if (res) {
                ...
                const token = { ...authToken, uid: userId };
...
export const doLogin = async (nickname: string) => {
  // 2. 获取engine实例
  ...
  try {
    ...
    // 3. 调用初始化方法
    setConfig(roomEngineInstance, userId, appKey, appId, signSecret);
    // 4. 登录
    await roomEngineInstance.auth(userId);
    window.engineAuthed = true;
  } catch (err) {
    console.error(err);
    throw err;
  }
