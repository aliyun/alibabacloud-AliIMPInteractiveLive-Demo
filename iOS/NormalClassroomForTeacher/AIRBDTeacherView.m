...
- (void) createRoomWithConfig:(AIRBRoomEngineConfig *)config userID:(NSString *)userID completion:(void (^)(NSString * _Nonnull))onGotRoomID {
    ...
    s = [NSString stringWithFormat:@"%@?appId=%@&templateId=%@&title=%@&notice=%@&roomOwnerId=%@", path, self.config.appID, templateId, [AIRBUtility encodeToPercentEscapeString:title], [AIRBUtility encodeToPercentEscapeString:notice], self.userID];
