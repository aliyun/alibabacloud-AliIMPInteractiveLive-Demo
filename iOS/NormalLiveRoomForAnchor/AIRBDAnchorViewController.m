...
- (void) createRoomWithCompletion:(void(^)(NSString* roomID))onGotRoomID {
    ...
    s = [NSString stringWithFormat:@"%@?appId=%@&templateId=%@&title=%@&notice=%@&roomOwnerId=%@", path, self.roomModel.config.appID, templateId, [AIRBUtility encodeToPercentEscapeString:self.roomModel.title], [AIRBUtility encodeToPercentEscapeString:self.roomModel.notice], self.roomModel.userID];
